module RISC_V_Processor(
  ...
  output wire [1:0] FPUException 
);
  // Wire declarations
  wire [63:0] PC_In, NextPC_In, BranchPC_In, imm_data, ReadData1, ReadData2, writeData, ReadData3, MemRead_Data, updatedData1, updatedData2;
  //wire [63:0] PC_Out, ALUResult; 
  ...
  //wire [31:0] FPUResult;          // FPU output
  wire FPUEnable;   // Control and exception signals for FPU
  //wire FPUException;
  wire [63:0] FinalResult;        // Final result after ALU and FPU selection
  wire [31:0] FPUOperandA = updatedData1[31:0];
  wire [31:0] FPUOperandB = ReadData3[31:0];
  wire shouldBranch;
  ...
  Control_Unit ct(.Opcode(opcode), .Branch(Branch), .MemRead(MemRead), .MemtoReg(MemtoReg),
                  .MemWrite(MemWrite), .ALUSrc(ALUSrc), .RegWrite(RegWrite),
                  .ALUOp(ALUOp), .FPUEnable(FPUEnable)); // Include FPUEnable
  ...
  FPU fpu(.opA(FPUOperandA), .opB(FPUOperandB), .FADD_FSUB(Operation[0]), .result(FPUResult), .exception(FPUException));
  mux aluFpuMux(.in1(ALUResult), .in2({32'b0, FPUResult}), .sel(FPUEnable), .out(FinalResult));

  // Memory Stage
  MEMRegister memReg(.clk(clk), .reset(reset), .PC_in(BranchPC_In),
                     .aluResult_in(FinalResult), .data2_in(updatedData2),
  ...
  mux memRegMux(.in1(FinalResult), .in2(MemRead_DataWB), .out(writeData), .sel(MemtoRegWB));
endmodule
...
//  wire Branch, MemRead, MemtoReg, MemWrite, ALUSrc, RegWrite, zero;
//  wire [31:0] FPUResult; // FPU output
//  wire FPUEnable, FPUException; // Control and exception signals for FPU
//  wand shouldBranch;

//  // Additional FPU wires
//  wire [31:0] FPUOperandA = updatedData1[31:0];
//  wire [31:0] FPUOperandB = ReadData3[31:0];
...
//  // Add multiplexer for ALU and FPU results
//  mux aluFpuMux(.in1(ALUResult), .in2({32'b0, FPUResult}), .sel(FPUEnable), .out(ALUResult));

//  //MEMRegister and other modules remain unchanged...
